...
use Kinetis\Console\McpServeCommand;
...
final class McpServeCommandTest extends TestCase
{
    ...
    private function serve(array $messages): array
    {
        ...
        $exit = new McpServeCommand($app->createRequestScope(), $this->projectRoot, $input, $output)->serve();
    ...
    /**
     * A project with no tools of its own still answers — with Kinetis's
     * own documentation resources, which are discovered from the
     * framework's namespace rather than the project's.
     */
    public function test_lists_tools_and_resources_for_a_project_with_none_of_its_own(): void
    {
        ...
        self::assertNotEmpty($responses[1]['result']['resources']);
    }
