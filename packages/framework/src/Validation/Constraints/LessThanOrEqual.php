...
/**
 * An inclusive bound a numeric value must not exceed.
 *
 * A non-finite threshold is refused at construction. JSON spells neither
 * INF nor NAN, so a `maximum` carrying one has no document it could be
...
final readonly class LessThanOrEqual implements Constraint
{
    public function __construct(
        ...
        if (is_float($threshold) && !is_finite($threshold)) {
            throw new InvalidArgumentException(
                'LessThanOrEqual threshold must be a finite number, ' . var_export($threshold, true) . ' given.',
            );
        }
    }
