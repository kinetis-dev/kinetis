...
/**
 * An exclusive bound a numeric value must be less than.
 *
 * A non-finite threshold is refused at construction. JSON spells
 * neither INF nor NAN, so an `exclusiveMaximum` carrying one has no
    ...
    public function __construct(
        ...
        if (is_float($threshold) && !is_finite($threshold)) {
            throw new InvalidArgumentException(
                'LessThan threshold must be a finite number, ' . var_export($threshold, true) . ' given.',
            );
        }
    }
