 ...
 * Channels wire protocol's own grammar — used by
 ...
 * own public methods — the two that sign a subscription and the one
 * that triggers an event alike, each rejecting a malformed direct call
 * with a thrown `InvalidPusherProtocolValueException` — so a caller is
 * held to the identical rule regardless of which boundary it reaches.
 *
 ...
 * package's own HMAC signing algorithm was verified against. The
 * `$channelName` check applies to the **full** channel name, including
 * its `private-`/`presence-` prefix — that's what the real Pusher
 * protocol validates against, and it's also the exact string this
 * package signs over and names in a trigger.
 */
