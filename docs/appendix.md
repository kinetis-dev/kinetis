...
- `HttpCache` / `McpCache` / `OpenApiCache` / `CommandCache` / `EventCache` — the five independent artifacts. `CacheStore` writes them via atomic tmp-file + `rename()`, reads via `require`. An object anywhere in an artifact (in practice, a constructor default like `new DateTimeImmutable()` captured into a plan) fails the write with `Exception\CacheWriteException::unexportableObject()` naming the path to it — `var_export()` would render it as a `::__set_state()` call the reload can't replay, so a clear build-time error replaces a corrupt artifact.
