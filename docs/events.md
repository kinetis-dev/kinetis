...
| `Kinetis\Queue\Events\JobReleased` | `kinetis/queue` | A job's `handle()` throws but attempts hasn't reached the effective cap yet, so it goes back on the queue for another try after the worker's backoff delay. | `class`, `queue`, `attempts`, `exception` — no delay, which is a floor the backend may exceed; the failure log line reports it |
