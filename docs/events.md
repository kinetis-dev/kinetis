...
| `Kinetis\Queue\Events\JobReleased` | `kinetis/queue` | A job's `handle()` throws but attempts hasn't reached the effective cap yet, so it goes back on the queue for another try after the worker's backoff delay. | `class`, `queue`, `attempts`, `exception` — no delay; the failure log line reports it |
...
| `Kinetis\Migrations\Events\MigrationApplied` | `kinetis/migrations` | Once per migration `migrate` actually runs, in the order they ran. | `name`, `connection` (the connection whose database it ran on) |
| `Kinetis\Migrations\Events\MigrationRolledBack` | `kinetis/migrations` | `migrate:rollback` undoes a migration — never fired when there was nothing to roll back. | `name`, `connection` (the connection whose database it ran on) |
