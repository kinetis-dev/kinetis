...
- `Kinetis\QueryBuilder\Query` — a thin, parameterized SQL query builder, not an ORM (no relationships/migrations/change-tracking). One class works with either MySQL or Postgres via the shared `Amp\Sql\SqlLink` family (auto-detected instanceof `Amp\Mysql\MysqlLink`/`Amp\Postgres\PostgresLink`). `select()`/`selectRaw()`/`where()`/`orWhere()`/`whereIn()`/`whereRaw()`/`join()`/`leftJoin()`/`orderBy()`/`orderByRaw()`/`limit()`/`offset()`, terminal `get()`/`first()`/`count()` (optional `Hydrator`-based DTO mapping) and `insert()`/`insertGetId()`/`update()`/`delete()`. Accepts a plain pool or an in-flight `SqlTransaction`, so it composes inside `TransactionGuard::transaction()`. Every terminal method routes through one private `run()`, which skips the database's own prepared-statement round trip via `$link->query()` whenever safe — a query with nothing bound at all, or one where every bound value is provably safe to write directly into the SQL as a literal (see {doc}`query-builder`'s "How a value reaches the database" section) — falling back to `$link->execute()` otherwise.
